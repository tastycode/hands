...
UserSchema.statics.nearTo = function(location, query, radius) {
...
UserSchema.methods = {
  ...
  *
  */
  nearbyTasks: function(radius) {
    try {
      var Task = mongoose.model('Task');
      var area = { center: [this.location.lng, this.location.lat], radius: 10, unique: true };
      return Task.aggregate(
        {
         "$geoNear":{
          "uniqueDocs":true,
          "near": this.location.coordinates,
          "spherical":true,
          "distanceField":"d",
          "distanceMultiplier": 3959.0,
          "maxDistance": radius/3959.0,
